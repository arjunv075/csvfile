...
def parse_args():
    ...
    parser_retrieve = subparser.add_parser("retrieve" ,help="retrieve data from database")
    ...
    parser_retrieve.add_argument("--qrcode",help="Generate qrcode file",action="store_true",default=False)
    ...
    parser_generate.add_argument("--qrcode",help="Generate qrcode file",action="store_true",default=False)
    
    parser_initleave = subparser.add_parser("initleave",help="Input data into leaves table")
    parser_initleave.add_argument("date", help="date of the employee's leave (Date format is : YYYY-MM-DD)")
    ...
    parser_initretrieve_leave = subparser.add_parser("retrieve_leave",help="Obtain data regarding leaves of employee")
    parser_initretrieve_leave.add_argument("employee_id", help="employee id of whome the leave data needs to be retrieved")
    
    parser_initcsv = subparser.add_parser("retrieve_csv",help="Generate details of employees leaves on csv file")
    parser_initcsv.add_argument("-f","--filename",help="Provide file name for generating csv, only file name and no file extention is needed",action='store',default="leave")
  ...
  try:
    with open(args.file,'r') as f:
      ...
      for last_name,first_name,title,email,phone in reader:
        designation_query = sa.select(models.Designation).where(models.Designation.title==title)
        designation = session.execute(designation_query).scalar_one()
    ...
    raise HRException("data provided already exists")
...
def retrieving_data_from_database(args):
  ...
  try:
    ...
    x=session.execute(employee_data_query).fetchall()
    session.commit()
...
def generate_vcard_file(args):
  ...
  try:
    ...
    data = session.execute(employee_data_query).fetchall()
    details = []
...
def retrieve_leave_data(args):
  ...
  try:
    ...
    if data == []:
       ...
       leaves = session.execute(leave_data_query).fetchall()
...
def generate_leave_csv(args):
  ...
  with open(f"{args.filename}.csv","w") as f:
    ...
    data.writerow(a)
  f.close()
  total_employee_count_query = (
            sa.select
            (sa.func.count(models.Employee.empid))
            )
  x = session.execute(total_employee_count_query).fetchall()
  for i in x:
    for j in i:
      count = j
  for i in range (1,count+1):
    employee_leaves_query = (
              sa.select(models.Leaves.empid)
              .join(models.Employee,models.Employee.empid==models.Leaves.empid)
              .where(models.Employee.empid==i)
             )
    data = (session.execute(employee_leaves_query).fetchall())
    if data == []:
      ...
      n = session.execute(employee_details_query).fetchall()
      ...
      m = session.execute(leave_count_query).fetchall()
