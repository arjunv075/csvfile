...
def parse_args():
    ...
    parser_initdb = subparser.add_parser(
        "initdb", help="Initialization of table in the database"
    )
    ...
    parser_retrieve.add_argument(
        "-i",
        "--id",
        help="ID of the employee whose data needs to be generated",
        type=str,
    )
    parser_retrieve.add_argument(
    ...
    parser_generate = subparser.add_parser("vcf", help="Generate vcards for the data")
    ...
    parser_initleave.add_argument("date", help="date of the employee's leave")
    ...
    parser_initrtrlv = subparser.add_parser(
        "rtrleave", help="Input data into leaves table"
    )
...
Phone       : {phone_number}"""
    ...
    if args.qrcd:
        ...
        with open(f"vcf_files/{email}.qr.png", "wb") as f:
            f.write(imp_qrcode)
        logger.info(f"created qr code of {email}")
    conn.close()
    ...
    for i in range(0, args.number):
        ...
        for lastname, firstname, title, email, phone_number in details:
            ...
            if args.qrcd:
                ...
                with open(f"vcf_files/{email}.qr.png", "wb") as f:
                    f.write(imp_qrcode)
        logger.info(f"created qr codes of{email}")
    logger.info("created vcards")
    conn.close()
